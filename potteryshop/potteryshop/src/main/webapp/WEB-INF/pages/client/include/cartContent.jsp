						...
						<tr class="cart_line" id="item${sanpham.id}">
							<div>
								...
								<td><b><span class="total" id="item${sanpham.id}_total"
										name="total_price">${sanpham.donGia*quanity[sanpham.id]}</span>
										VND</b></td>
								<td class="cart-img"><a class="btn btn-danger"
									onClick="deleteFromCart(${sanpham.id})"><span
						...
						<tr class="cart_line" id="itemNew${sanpham.id}">
							<div>
								...
								<td style="text-align: center; mergin-top: -52px;">
									...
									<p class="cart_ten">
										<a href="<c:url value="/sp?id=${sanpham.id}" />"><h1>NEW
											</h1>${sanpham.tenSanPham} </a>
									</p>
									<p class="cart_masanpham">
										Mã sản phẩm : <span>${sanpham.id}</span>
									</p>
								...
								<td>
								<c:choose>
										<c:when test="${fn:length(cartNew)>0 }">
											<input disabled class="input_num_cart" type="number"
												value="${quanityNew[sanpham.id]}" min="1"
												onChange="changeQuanityNew(${sanpham.id},this.value,${sanpham.donGia})">
								...
								<td><b><span class="total" id="itemNew${sanpham.id}_total"
										name="total_price">${sanpham.donGia*quanityNew[sanpham.id] }
									</span> VND</b></td>
								<td class="cart-img"><a class="btn btn-danger"
									onClick="deleteFromCartNew(${sanpham.id})"><span
										class="glyphicon glyphicon-trash"></span></a></td>
							</div>
						</tr>
					</c:forEach>
						...
						<c:if test="${checkEmpty != 0 }">
							<td colspan="3"><a class="btn btn-primary"
								...
								href="<%=request.getContextPath()%>/checkout"><span
									class="glyphicon glyphicon-check"></span> Thanh toán</a></td>
						</c:if>

						<c:if test="${checkEmpty == 0 }">
							<td colspan="3"><a class="btn btn-primary"
								href="<%=request.getContextPath()%>/">Mua thêm sản phẩm khác</a>
							</td>
						</c:if>
						<td colspan="4">
